...
| **ИТОГО** | **14 106,7** | **24 454,2** | **64,3 ГБ/с** | **109,7 ГБ/с** | **~4,0 ПБ** |
...
| **Итого (RPS пик)**  | **25 164.202** | **7 494.9**            | **7 494.9**            | **7 494.9**            | **6 347.2**            |

---

DNS балансировка Geo-DNS, что бы разделить трафик на 2 региона. Если сервер в Новосибирске выйдет из строя будет выдан DNS Москвы.
...
L4 - Virtual Server via IP Tunneling (Используем на LVS (Linux Virtual Server))
- Позволяет решить проблему того что выходящий трафик намного больше чем входящий. Балансировщик энкапсулирует пакет и его уже отдаёт сервер на который балансировщик этот пакет отправил.
