...
<img width="2728" height="8804" alt="image" src="https://github.com/user-attachments/assets/5b642ded-5c2e-4373-9fe5-83063e989968" />
...
| Табилца          | Чтение (RPS) | Запись (RPS)  | CPU |
|-------------------|--------------|---------------|-----|
| пост              | 9 920        | 2             | 40  |
| профиль           | 14 108       | 1             | 65  |
| сессия            | 9 920        | 30            | 2   |
| комментарии       | 13 450       | 16            | 55  |
| рекомендации      | 9 920        | —             | 200 |
| метрика поиска    | -            | 165           | 1   |
| метрика просмотра | -            | 9 300         | 2   |
| подписка          | 20820        | 2             | 85  |
| фото              | 42 000       | 11.2          | 360 |
| поиск             | 100          | 2             | 4   |
| итого             |              |               | 800 |

необходимо обеспечить загруженность 40% для возможности расширения в будующем => 800 / 0,4 = 2 000 ядер
...
по бенчмарку [22] получаем 4000 qps чтения с записью репликаций на 16 ядер т.е. 250 на ядро.

на основе ссылки https://stackoverflow.com/questions/67478213/is-200k-rows-second-clickhouse-max-performance - 28 ядер и 200к вставок получаем 7000 вставок в clickhouse 1 ядром

https://amperecomputing.com/briefs/minio-on-single-node-brief здесь минио выдаёт 5500 ответов по 2 мб на 56 ядрах => 100 запросов на ядро

Из https://www.techempower.com/benchmarks/#section=data-r23 go-pgx-prefork выдаёт 300к запросов на 56 ядрах => 5000 rps на одно ядро для сервиса


| Действие             | RPS пик | CPU |
|----------------------|---------|-----|
| Лента постов         | 620     | 1   |
| Чтение постов        | 9 300   | 2   |
| Написание постов     | 2       | 1   |
| Коментарии           | 16      | 1   |
| Коментарии(чтение)   | 4 150   | 1   |
| Рекомендации         | 10 900  | 3   |
| Регистрация          | 0.002   | 1   |
| Подписка             | 1.2     | 1   |
| Поиск                | 165     | 1   |
| Авторизация          | 6       | 1   |
...
Опять хотим получить загрузку 40% => 13 / 0,4 = 33 ядра
...
Для фото берём ssd на 15 тб https://serverflow.ru/catalog/komplektuyushchie/ssd-nakopiteli/micron-7450-pro-15-36tb-2-5-u-3/

282 / 15 = 18,8 ~= 19. Для запаса берём ещё 6 итого 25 дисков

343 000 * 25 = 8,6 млн

Для БД остальных берём optane https://serverflow.ru/catalog/komplektuyushchie/ssd-nakopiteli/ssd-nakopitel-intel-optane-dc-p4800x-1-5tb-ssdpe21k015ta01/ для борьбы с мелкоблочкой.

Все основные б весят ~= 6 Тб => 6 / 1,5 = 4 диска

берём 4 / 0,4 = 10 дисков

10 * 98 000 = 1 млн

Сервер xFusion 1288H V7 8SFF - для БД и сервисов (1 828 200р) https://servermall.ru/catalog/servers/xfusion-1288h-v7-8sff/
- СPU 2x Intel Xeon Gold 6430 (32C 60M Cache 2.10 GHz)
- RAM 12x 64GB DDR5 RDIMM 4800MHz (Поддержка до 32 DIMM портов DDR5)
- БП 2x xFusion 2000W
- RAID RAID xFusion 9560-8i-PCIe (4GB) SAS/SATA
- HDD 2x SSD 480GB SATA 2,5" (до 8 HDD 2.5'' SFF)
- Net 2 port 25Gb/s SFP28 PCIe 3.

2000 ядер для БД и минио = 2 000

2 000 / 64 ~= 32 сервера + 9 в запасе + 3 (по 1 сервер на каждый дц для Сервисов - 32 ядра + 2 nginx по 8 ядер)

44 * 1 828 200 = 80,5 млн рублей

итого 8,6 + 1 + 64 = 90,1 млн рублей на 1 ДЦ

Надо обеспечить что бы 3 ДЦ выдерживали падение четвёртого. По таблице нагрузки на БД видно что запись почти не влияет на кол-во cpu. Мы хотим что бы minio хранилось в 2 ДЦ, а значит

73,6 + 8,6 + 3 (ssd для БД) = 85,2 млн рублей. Мы изначально рассчитвывали что бы пики занимали 0,4 всех cpu. При отключении 1 ДЦ у нас будет 0,5-0,6 нагрузка на cpu в пиках.

S3 хранилище 250 Тб от яндекса https://yandex.cloud/ru/prices стоит ~= 750 000 р / месяц. => диски 9,6 млн / 750тыс = 12,8 ~= 13 месяц окупятся

- сервер selectel - AL82-NVMe-10GE - 64 000 / месяц https://selectel.ru/services/colocation/server-rack/
- За наш серевер платим аренду (350 000 за стойку) / 32 + (225 000 за дополнительные 10кв) / 32 = 11 000 + 7 000 = 18 000р
- Разница 64 000 - 18 000 = 46 000 в месяц
- 1 828 000 / 46 000 = 40 месяцев и мы выйдем в плюс
