...
L7 - HTTP Reverse proxy (Используем Nginx снаружи кубера)
...
|profile_metric        | Postgresql + Redis | В redis храним real time статистику, а в postgresql уже обновлённую для сохранности|
...
- пост
  ...
  - status - для быстрого отсеивания удалённых или непромодерированных постов btree
    - вес индекса 8 * 122 000 000 = 1 гб
- сессия
...
- профиль
  - id - стандартный индекс btree
    - вес индекса 8 * 100 000 000 = 800 мб
  - username - для проверки существования логина hash
    - вес индекса 10 (средний размер логина) * 100 000 000 = 1 гб
...
- Метрики по постам и профилям в отдельных таблицах
...
- profile_metric: 32 * 122 000 000 = 4 гб

**Шардирование и резервирование СУБД (потаблично)**

по бенчмарку pgbnech[22] средний сервер выдаёт 500 000 tps на чтение и 8 000 tps на запись
...
все бекапы с реплик:
- физические каждую неделю - postgresql, clickhouse
- снапшоты redis - каждый час
- elastic - снапшоты каждую неделю
- clickhouse - бекап с помощью clickhouse-backup каждую неделю
...
|--------------------------|--------------------|--------------------------------------------------------|
| **Golang**               | Backend            | Высокая производительность, статическая типизация, простота развёртывания (один бинарник)|
| **TypeScript**           | Frontend           | Снижает количество ошибок из-за строгой типизации |
| **React**                | Frontend           | Компонентный подход, богатая экосистема |
| **Ant Design**           | Frontend           | UI-компоненты для веб-приложений на React, самые популярные |
| **PostgreSQL**           | БД                 | Надёжность, ACID, сложные типы (JSONB, массивы), масштабируемость |
| **pgbouncer**            | Прокси             | Обеспечение балансировки нагрузки для postgresql |
| **Elasticsearch**        | БД                 | Полноценный поиск по тексту в реальном времени |
| **Redis**                | БД                 | Хранение данных в памяти, высокая скорость, очереди |
| **ClickHouse**           | БД                 | Колоночная бд, оптимизирован под аналитические запросы, эффективная ставка |
| **Minio**                | БД                 | S3 для хранения файлов, масштабируемо, легче в настройке чем ceph, совместимость с Amazon S3 |
| **Kubernetes**           | Разработка         | Стандарт для развёртывания и управления распределёнными системами, масштабирование и надёжность |
| **Docker**               | Разработка         | Создание изолированных сред. Используетс с kubernetes |
| **Nginx**                | Прокси             | Высокая производительность, надёжность, SSL, обновление конфига без остановки приложения |
| **go-redis**             | Backend            | Официальный клиент для redis, поддержка кластера |
| **go-elasticsearch**     | Backend            | Официальная клиент Elastic, поддержка всех API |
| **pgx**                  | Backend            | Высокая производительность, поддержка PostgreSQL-специфичных функций (JSONB) |
| **clickhouse-go**        | Backend            | Официальный клиент clickhouse, поддержка bulk-загрузки |
| **minio-go**             | Backend            | Официальные клиент |
| **Python**               | ML                 | Основной язык для ML разработчиков |
| **implicit**             | ML                 | Для item2item, работает на GPU |
| **efanna**               | ML                 | Одна из самых быстрых библиотек KNN (по их бенчмаркам) |
| **Gitea**                | Разработка         | Личный git сервер с поддержкой ci cd, ревью кода и команд |
| **Vault**                | Backend            | Хранилище для хранение секретов |
| **Prometheus**           | Backend            | Сбор метрик сервисов, |
| **Grafana**              | Backend            | Визуализация метрик, алёрты, уведомления |
| **Logstash**             | Backend            | Сбор логов и передача в elasticsearch |

Основной принцип — CQRS подразумевает разделение пайплайнов для чтения и записи в БД. Модели для записи синхронизируются с моделью для чтения.

Плюсы:

- Возможность управлять нагрузкой на запись в БД.
- Запросы чтения не зависят от записывающих команд.

Если master упадёт на слейва pgbouncer не переключится так как не знает что он новый мастер. Kafka сложнее ложится так как нужно что бы 1/3 серверов умерла. 

## 9. Схема проекта

<img width="750" height="2500" alt="image" src="https://github.com/user-attachments/assets/db9cbd83-a7aa-45fc-ba7d-f0629e10f579" />

API-gateway - общение между сервисами

Плюсы:
- Единая точка контроля над трафиком клиентов.
- Возможность исполнять несколько запросов в
рамках одного обращения.
- Возможность настраивать выполнение запросов
для конкретного клиента.
- Изоляция клиентов от логики микросервисов.

CQRS - разделение пайплайнов для чтения и записи в БД. Модели для записи синхронизируются с моделью для чтения.

Плюсы:
- Возможность управлять нагрузкой на запись в БД.
- Запросы чтения не зависят от записывающих команд.

SAGA - транзакции для разных БД.

Плюсы:
- Поддержка консистентности данных в различных БД без использования распределенных транзакций.



## 10. Обеспечение надёжности

| Технологии                     | Механизм резервирования | 
|--------------------------------|-------------------------|
| BGP Anycast                    | Распределение трафика между 3 ДЦ. При сбое 1 ДЦ, трафик автоматически переключается на 2 других | 
| Latency-based DNS              | Раcпределение между 2 ip - Москва(3 ДЦ) и Новосибирск (1 ДЦ). Если ДЦ в Новосибирске упадёт DNS переключится на московский ip|
| NGINX                          | 2(2N) реплики на ДЦ. Через VPPR резервный подменит основной в случае отказа | 
| Микросервисы                   | По 1 реплике в ДЦ. Kubernetes Распределения подов по узлам кластера с автоматическим перезапуском на резервных узлах при сбоях. Graceful Shutdown - не принимаем новые и завершаем принятые запросы. | 
| Minio                          | Делаем 2 копии данных на разных ДЦ | 
| PostgreSQL                     | Ассихронная репликация. Реплики в разных ДЦ. Еженедельные бекапы. pgBouncer для мультиплексирования подключений. | 
| Redis                          | Redis с шардированием с помощью redis-cluster и репликацией | 
| ClickHouse                     | Реплика ClickHouse на другом ДЦ | 
| Prometheus, Grafana            | Сбор метрик - потребление cpu, памяти, сети, RPS. Алёрты от grafana |
| Сервера                        | Иметь в запасе отдельно несколько серверов, дисков, сетевых карт для замены. Стандартизация серверов |
| Надёжность хранения            | RAID 6 - как RAID 5 но информация о чётности записывается сразу на 2х дисках. Позволяет потерять 2 диска |
| graceful degradation           | При отказе рекомендательного сервиса брать самые популярные и созданные за 30 дней. При отказе комментариев ставить заглушку. При отказе постов новые посты пишутся в kafka. Аналогично с новыми пользователями. Если отказали сессий показывать сайт относительно гостя. При отказе MASTER - только читаем из SLAVE и складываем записи в kafka |

CQRS - разделение пайплайнов для чтения и записи в БД. Модели для записи синхронизируются с моделью для чтения. При выходе из строя мастера - данные будут храниться в kafka и при поднятии запишутся
...
В моём случае рассматриваем с Оркестратором - оркестратор знает сценарий целиком и по шагам рассылает команды сервисам: что делать дальше. Он вызывает Service A, затем Service B, и т.д., и в случае ошибки также инициирует компенсирующие действия. Оркестратор упрощает понимание последовательности (вся логика сосредоточена в одном месте), но вводит дополнительный компонент и точку контроля. В нашем рассылка команды происходит через очередь profile_update. Успех или неуспех от сервисов приходит в очередь saga_status. При ошибке рассылает в очередь profile_update сообщения об компенсации.

Graceful degradation (рекомендер) - API gateway в случае ошибки / таймаута / медленного ответа рекомендера - автоматически переключается на fallback (топ-50)

## 11. Список серверов

| Действие             | RPS пик |Какие таблицы|
|----------------------|---------|----|
| Лента постов         | 620     |пост, профиль, статистика поста, рекомендации, сессия, фото|
| Чтение постов        | 9 300   |пост, профиль, статистика поста, комментарии, рекомендации, метрики профиля, сессия, фото|
| Написание постов     | 2       |пост, профиль, сессия, фото|
| Коментарии           | 16      |профиль, коментарии, сессия, метрики профиля|
| Коментарии(чтение)   | 4 150   |коментарии|
| Регистрация          | 0.002   |профиль|
| Подписка             | 1.2     |профиль, сессия, подписка, метрики профиля|
| Поиск                | 165     |пост, т|
| Авторизация          | 6       |сессия, профиль|


| Табилца           | Чтение (RPS) | Запись (RPS)   | CPU | ДЦ1 | ДЦ2 | ДЦ3 | ДЦ4 |
|-------------------|--------------|---------------|-----|-----|-----|-----|-----|
| пост              | 9 920        | 2             | 40  | 10  | 10  | 10  | 10  |
| профиль           | 14 108       | 1             | 68  | 17  | 17  | 17  | 17  |
| сессия            | 9 920        | 30            | 4   | 1   | 1   | 1   | 1   |
| комментарии       | 13 450       | 16            | 56  | 14  | 14  | 14  | 14  |
| рекомендации      | 9 920        | —             | 16  | 4   | 4   | 4   | 4   |
| метрика поиска    | -            | 165           | 4   | 1   | 1   | 1   | 1   |
| метрика просмотра | -            | 9 300         | 4   | 1   | 1   | 1   | 1   |
| подписка          | 20820        | 2             | 88  | 22  | 22  | 22  | 22  |
| фото              | 36 000       | 11.2          | 360 | 90  | 90  | 90  | 90  |
| поиск             | 100          | 2             | 4   | 1   | 1   | 1   | 1   |
| итого             |              |               | 644 | 161 | 161 | 161 | 161 |

необходимо обеспечить загруженность 40% для возможности расширения в будующем => 644 / 0,4 = 1 610 ядер или 161 / 0,4 = 402 ядер на ДЦ

по фото: 9 300 + 620 = 9 920 , но фото не одно по 6 штук в среднем => 59 520 ~= 60 000, но так как в среднем читают 60% поста, то получим 36 000 rps

- postgresql: по бенчмарку pgbench[22] получаем 4000 qps чтения с записью репликаций на 16 ядер т.е. 250 на ядро.
- clickhouse: на основе ссылки[31] - 28 ядер и 200к вставок получаем 7000 вставок в clickhouse 1 ядром
- minio:[30] здесь минио выдаёт 5500 ответов по 2 мб на 56 ядрах => 100 запросов на ядро
- go:[29] выдаёт 300к запросов на 56 ядрах => 5000 rps на одно ядро для сервиса


| Действие             | RPS пик | Логика | CPU | Память |
|----------------------|---------|--------|-----|--------|
| Лента постов         | 620     |Лёгкая  | 1   | 1 ГБ   |
| Чтение постов        | 9 300   |Лёгкая  | 2   | 1 ГБ   |
| Написание постов     | 2       |Лёгкая  | 1   | 1 ГБ   |
| Коментарии           | 16      |Лёгкая  | 1   | 1 ГБ   |
| Коментарии(чтение)   | 4 150   |Лёгкая  | 1   | 1 ГБ   |
| Рекомендации         | 10 900  |Тяжёлая | 218 | 218 ГБ |
| Регистрация          | 0.002   |Средняя | 1   | 1 ГБ   |
| Подписка             | 1.2     |Лёгкая  | 1   | 1 ГБ   |
| Поиск                | 165     |Лёгкая  | 1   | 1 ГБ   |
| Авторизация          | 6       |Лёгкая  | 1   | 1 ГБ   |
| NGINGX               | -       |Средняя | 8   | 8 ГБ
| Итого                |         |        | 238 | 238 ГБ |

Опять хотим получить загрузку 40% => 230 / 0,4 = 595 ядер и 230 / 0,4 = 595 ГБ

|Таблица     |Колонка                    |Вес    |
|------------|---------------------------|-------|
|Пост        | id                        |1 ГБ   |
|Пост        | author_id                 |0,08 ГБ|
|Пост        | status                    |1 ГБ   |
|Комментарии | id                        |16 ГБ  |
|Комментарии | post_id, created_at       |32 ГБ  |
|Комментарии | id, porent_id, created_at |48 ГБ  |
|Подписка    | id                        |2,4 ГБ |
|Подписка    | author_id, subscriber_id  |4,8 ГБ |
|Выплаты     | id                        |14,5 ГБ|
|Профиль     | id                        |0,8 ГБ |
|Профиль     | username                  |1 ГБ   |
|Метрик. проф| id                        |0,8 ГБ |
|Метрик. проф| profile_id                |0,8 ГБ |
|Метрик. пост| id                        |1 ГБ   |
|Метрик. пост| post_id                   |1 ГБ   |
|Поиск       | id                        |1 ГБ   |
|Поиск       | title                     |12 ГБ  |
|Поиск       | author                    |2 ГБ   |
|Сессии      | REDIS                     |14,4 ГБ|
|Метрик. проф| REDIS                     |1 ГБ   |
|Метрик. пост| REDIS                     |1 ГБ   |
|Итого       |                           |135 ГБ |
...
|profile       |68,9 ГБ|
|view_metrics  |890  ГБ|
|recommendation|53   ГБ|
|search_metrics|68,3 ГБ|
|subscription  |67,2 ГБ|
|payment       |0,5  ГБ|
|comment       |2,11 ТБ|
|photo         |282  ТБ|
|post_metric   |4,88 ГБ|
...
Для фото берём ssd на 15 тб [27]

282 ТБ / 15 ТБ = 18,8 ~= 19. Для запаса берём ещё 6 итого 25 дисков. Берём ещё 10 дополнительных ssd для raid 6 (в сервер помещается 8 ssd. 8 - 2(на raid 6) = 6. 25 / 6 ~= 5. 5 * 2 = 10). Итого 35 дисков

343 000 * 35 = 12 млн

Для БД остальных берём optane [28] для борьбы с мелкоблочкой.

Все основные б весят ~= 6 Тб => 6 / 1,5 = 4 диска

берём 4 / 0,4 = 10 дисков => + 4 диска для RAID 6 (в сервер 6 реальных + 2 диска для raid 6) = 14 дисков

14 * 98 000 = 1,4 млн

Сервер xFusion 1288H V7 8SFF - для БД и сервисов (1 828 200р) [26]
- СPU 64 ядра - 2x Intel Xeon Gold 6430 (32C 60M Cache 2.10 GHz)
- RAM 768 ГБ - 12x 64GB DDR5 RDIMM 4800MHz (Поддержка до 32 DIMM портов DDR5)
- БП 2x xFusion 2000W
- RAID RAID xFusion 9560-8i-PCIe (4GB) SAS/SATA
- HDD 2x SSD 480GB SATA 2,5" (до 8 HDD 2.5'' SFF)
- Net 2 port 25Gb/s SFP28 PCIe 3.

На каждый ДЦ
- 402 + 595 = 997 ядер => 997 / 64 ~= 16 серверов * 1 828 200 = 30 млн
- 338 + 595 = 951 ГБ - вмещаются в 2 сервера а мы берём 16 по ядрам. Т.е. во всех ДЦ реплики поместятся по памяти в 2 сервера. Проблем с мастером (ещё + 338 ГБ) и репликой в одном из ДЦ не будет!
- 14 дисков (1,4 млн) для БД
- Итого 32 млн на ДЦ

- Отдельно считаем на 2 ДЦ Minio это + 12 млн * 2 = 24 млн на диски - тут уже учтены 6 дисков для запаса на 6 лет работы сервиса
- Отдельно берём 8 серверов - по 2 сервера по ДЦ для замены это ещё 15 млн

ИТОГО: 32 * 4 + 24 + 15 = 167 млн рублей

Надо обеспечить что бы 3 ДЦ выдерживали падение четвёртого - мы изначально рассчитвывали что бы пики занимали 0,4 всех cpu. При отключении 1 ДЦ у нас будет 0,5-0,6 нагрузка на cpu в пиках.

S3 хранилище 250 Тб от яндекса [25] стоит ~= 750 000 р / месяц. => диски 12 млн / 750тыс = 16 месяц окупятся


PCL38-NVMe-10GE
- 2 × AMD EPYC 7513 (32x2.6 GHz SMT)
- 512 ГБ DDR4 ECC Reg
...
Рассчёт выгоды
- сервер selectel - PCL38-NVMe-10GE - 120 920 ₽/мес [23]
- За наш серевер платим аренду [24] (350 000 за стойку) / 32 + (225 000 за дополнительные 10кв) / 32 = 11 000 + 7 000 = 18 000 Руб
- Дополнительная амортизация 20% в год от сервера - 30 500 Руб
- Разница 120 920 - 18 000 - 30 000 = 72 000 в месяц
- 1 828 000 / 72 000 = 26 месяцев и мы выйдем в плюс

При условии что сервера меняем раз в 5 лет выгоднее хостить свои
...
23. https://selectel.ru/services/dedicated/?tab=configurator&c=309%3A2-54%3A8-378%3A1-127%3A4-81%3A1-308%3A1-172%3A1&simpleRamMode=true
24. https://selectel.ru/services/colocation/server-rack/
25. https://yandex.cloud/ru/prices
26. https://servermall.ru/catalog/servers/xfusion-1288h-v7-8sff
27. https://serverflow.ru/catalog/komplektuyushchie/ssd-nakopiteli/micron-7450-pro-15-36tb-2-5-u-3/
28. https://serverflow.ru/catalog/komplektuyushchie/ssd-nakopiteli/ssd-nakopitel-intel-optane-dc-p4800x-1-5tb-ssdpe21k015ta01/
29. https://www.techempower.com/benchmarks/#section=data-r23 go-pgx-prefork
30. https://amperecomputing.com/briefs/minio-on-single-node-brief
31. https://stackoverflow.com/questions/67478213/is-200k-rows-second-clickhouse-max-performance
...
[16]: https://www.cnews.ru/news/line/2022-12-23_dzen_podvel_itogi_2022_goda
[17]: https://supa.ru/blog/posts/v-2019-ghodu-iandieks-dzien-udvoil-mnoghiie-pokazatieli
[18]: https://ru.wikipedia.org/wiki/%D0%93%D0%BE%D1%80%D0%BE%D0%B4%D0%B0_%D0%A0%D0%BE%D1%81%D1%81%D0%B8%D0%B8_%D1%81_%D0%BD%D0%B0%D1%81%D0%B5%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5%D0%BC_%D0%B1%D0%BE%D0%BB%D0%B5%D0%B5_500_%D1%82%D1%8B%D1%81%D1%8F%D1%87_%D1%87%D0%B5%D0%BB%D0%BE%D0%B2%D0%B5%D0%BAhttps://ru.wikipedia.org/wiki/%D0%93%D0%BE%D1%80%D0%BE%D0%B4%D0%B0_%D0%A0%D0%BE%D1%81%D1%81%D0%B8%D0%B8_%D1%81_%D0%BD%D0%B0%D1%81%D0%B5%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5%D0%BC_%D0%B1%D0%BE%D0%BB%D0%B5%D0%B5_500_%D1%82%D1%8B%D1%81%D1%8F%D1%87_%D1%87%D0%B5%D0%BB%D0%BE%D0%B2%D0%B5%D0%BA
[19]: https://habr.com/en/companies/skillfactory/articles/862130/
[20]: https://www.comnews.ru/content/211042/2020-10-21/2020-w43/magistralnye-seti-svyazi-rossii
[21]: https://leadmachine.ru/2020/08/05/content-marketing-vs-yandex-dzen/?utm_medium=organic&utm_source=yandexsmartcamera
[22]: https://openbenchmarking.org/result/2510061-NE-2510055NE79#r-7e3cefef86b702f438318e5f7cc7938646057dc9
